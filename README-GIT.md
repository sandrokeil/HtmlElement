...
Your first step is to establish a public repository from which i can pull your work into the master repository.
 ...
 2. Fork the HtmlElement repository (http://github.com/sandrokeil/HtmlElement)
 3. Clone your fork locally and enter it (use your own GitHub username in the statement below)

    ```sh
    % git clone git@github.com:<username>/HtmlElement.git
    % cd HtmlElement
    ```

 4. Add a remote to the canonical HtmlElement repository, so you can keep your fork
    up-to-date:

    ```sh
    % git remote add upstream https://github.com/sandrokeil/HtmlElement.git
    % git fetch upstream
    ```

## Keeping Up-to-Date

Periodically, you should update your fork to match the canonical HtmlElement repository. we have
added a remote to the HtmlElement repository, which allows you to do the following:
...
## Working on HtmlElement

When working on HtmlElement, we recommend you do each new feature or bugfix in a new branch. This simplifies the
...
To ssh://git@github.com/sandrokeil/HtmlElement.git
...
`https://github.com/sandrokeil/HtmlElement/commits/<branch>.atom`

where &lt;branch&gt; is a branch in the repository.

To subscribe to git email notifications, simply watch or fork the HtmlElement repository on GitHub.
